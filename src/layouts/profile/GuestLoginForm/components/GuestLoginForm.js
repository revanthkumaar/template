...
const FORM_VALIDATION = Yup.object().shape({
	...
	localGuardianName: Yup.string().matches(/^[aA-zZ\s]+$/, 'Invalid Name ').required('Required'),
...
const GuestLoginForm = () => {
	...
	const handleClick = (id) => {
		...
		const bool = oneBuilding.filter((buildingData) => buildingData.buildingName == id.target.outerText);
	...
	const selectBed = (e) => {
		...
		const bedRent = availableBeds.filter((bed) => bed.bedId == e.target.outerText);
		console.log(bedRent);
							...
							{(formProps) => (
								<Form>
									<Grid container spacing={2}>
										...
										<Grid item xs={6}>
											<InputLabel id="demo-simple-select-labe"> Duration</InputLabel>
											<Select
												className={classes.root}
												name="duration"
												options={duration}
										...
										<Grid item xs={6}>
											<Textfield name="dueAmount" label="Default Rent" value={defaultRentofBed} />
											{/* <Select
